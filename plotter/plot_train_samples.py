...
for futu_path, orig_path in zip(futu,orig):
    ...
    for p, k in zip(weather_params, wvars):
        if k == '1':
            ...
            for n in range(plots_per_batch):
                print('plotting sample #' + str(n) + 'from batch')
                init = original[n,0,:,:,:].reshape([32,32])
                real = original[n,1,:,:,:].reshape([32,32])
                gen = forecast[n,1,:,:,:].reshape([32,32])
                ...
                plt.tight_layout()
                superendpath = source_path + endfile + p + '_step' + str(counter) + '_batch' + str(n) + '.png'
